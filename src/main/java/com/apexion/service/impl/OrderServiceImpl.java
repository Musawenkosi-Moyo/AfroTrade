...
public class OrderServiceImpl implements OrderService {
    ...
    public Set<Order> createOrder(User user, Address shippingAddress, Cart cart) {
        ...
        // Checking if the delivery address is already added to the customers address or
        // not
        ...
        Map<Long, List<CartItems>> itemsBySeller = cart.getCartItems().stream()
                .collect(Collectors.groupingBy(items -> items.getProduct().getSeller().getId()));

        Set<Order> orders = new HashSet<>();

        // Iterate through all items by seller, then create seperate order for each
        // seller
    ...
    public OrderItem getOrderItemById(Long id) throws Exception {
        return orderItemRepository.findById(id).orElseThrow(() -> new Exception("OrderItem list not found"));
    }
